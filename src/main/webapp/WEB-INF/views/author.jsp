...
<script>
	...
	function addContainerList() {
		...
		apps = eval(data);
		console.log(apps.fileStoreInfos.length);
		if (apps.fileStoreInfos.length > 9 * (number + 1)) {
			dataF = 9 * (number + 1);
		} else {
			page++;
			dataF = apps.fileStoreInfos.length;
			$.ajax({
				type : 'get',
		...
		for (i = number * 9; i < dataF; i++) {
			...
			vImg.src = "${resources}/files/"
					+ apps.fileStoreInfos[i].theme.iconPath;
