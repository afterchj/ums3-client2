...
public class MyEncryptedMappingJacksonJsonView extends AbstractView {
	...
	protected void writeContent(OutputStream stream, Object value, boolean prefixJson) throws IOException {
		...
		Object encrypted = valueMap.remove(Constants.BACKGROUND_ENCRPTION_KEY);
		...
		if(encrypted == null || !(boolean)encrypted){
			...
			try {
				stream.write(URLEncoder.encode(Encodes.encodeBase64(DESCoder.encrypt(bytes)), "utf-8").getBytes());
			} catch (Exception e) {
