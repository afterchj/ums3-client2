...
public class InfoController extends BaseDecodedController{
	...
	public String complete(@ModelAttribute("decodedParams") JSONObject params, ModelMap model) {
		...
		try {
			...
			infoManager.patch(OfferFactory.generateLoginedOffer(uid, token), tpadUser);
			if(StringUtils.isNotBlank(invitation) && !StringUtils.equals(invitation, uid)){
				Present present = submitUserInvitationTask(uid, token, invitation);
	...
	private Present submitUserInvitationTask(String uid, String token, String invitation) {
		Present present = null;
		try {
			AppUser user = queryManager.findByLoginName(Constants.APP_ID, invitation);
			if(user != null && user.getStatus() != Constants.DISABLED){
				UserCommitOffer offer = OfferFactory.generateUserCommitOffer(Constants.TASK_RECOMMEND, uid, token, "invi" + uid);
			...
			system.warn("method:submitUserInvitation,uid:" + uid + ",invitation:" + invitation + ",cause:taskRepeatException");
		} catch (Exception e) {
			system.error("method:submitUserInvitation,uid:" + uid + ",invitation:" + invitation, e);
		}
		return present;
	}
...
//	14	icon	用户头像URL地址	255	NULL
	...
	public String modify(@ModelAttribute("decodedParams") JSONObject params,
		...
		try {
			...
			infoManager.patch(OfferFactory.generateLoginedOffer(uid, token), appUser);
			model.put("result", ResultDict.SUCCESS.getCode());
		} catch (SystemAlgorithmException | ApplicationNotCorrectException e) {
			system.error(e);
			model.put("result", ResultDict.SYSTEM_ERROR.getCode());
	...
	private AppUser generateAppUser(JSONObject params) {
		...
		appUser.setCommunityLoginSuc(params.getBooleanValue("communityLoginSuc"));
		return appUser;
	}
	...
	public String commit(@ModelAttribute("decodedParams") JSONObject params,
		...
		try {
			...
			String token = params.getString("token");
			AppUser appUser = generateAppUser(params);
			AppUser submittedAppUser = infoManager.remoteSubmit(OfferFactory.generateLoginedOffer(uid, token), appUser);
			model.put("result", ResultDict.SUCCESS.getCode());
			...
			model.put("result", ResultDict.SYSTEM_ERROR.getCode());
			system.error("params:" + params.toJSONString() + "\t" + e.getMessage());
		}
		return null;
	}
	...
	public String changeMode(@ModelAttribute("decodedParams") JSONObject params,
		...
		try {
			...
			if(mode != null){
				infoManager.changeMode(OfferFactory.generateLoginedOffer(uid, token), mode);
			}
			if(mode2 != null){
				infoManager.changeMode2(OfferFactory.generateLoginedOffer(uid, token), mode2);
			}
			model.put("result", ResultDict.SUCCESS.getCode());
		} catch (SystemAlgorithmException | ApplicationNotCorrectException e) {
			system.error(e);
	...
	public String getInfo(@ModelAttribute("decodedParams") JSONObject params,
		...
		try {
			String uid = params.getString("uid");
			String token = params.getString("token");
			AppUser appUser = infoManager.getBaseUserInfo(OfferFactory.generateLoginedOffer(uid, token));
			model.put("result", ResultDict.SUCCESS.getCode());
