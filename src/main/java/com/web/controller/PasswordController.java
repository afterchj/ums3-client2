...
public class PasswordController extends BaseDecodedController {
    ...
    public String retrieve(@ModelAttribute("decodedParams") JSONObject
                                   params, ModelMap model) {
        try {
            String mobile = params.getString("mobile");
            String code = params.getString("code");
            String password = params.getString("password");
            accountManager.retrieveTpadPassword(OfferFactory
					.generateMobileVerifyOffer(mobile, MobileVerifyType
							.UPDATE, code), password);
            AppUser appUser = accountManager.loginByTpad(OfferFactory
					.generateUserLoginOffer(mobile, password));
    ...
    public String change(@ModelAttribute("decodedParams") JSONObject params,
        ...
        try {
            ...
            accountManager.changeTpadPassword(OfferFactory
					.generateLoginedOffer(uid, token), oldPassword,
					newPassword);
            model.put("result", ResultDict.SUCCESS.getCode());
