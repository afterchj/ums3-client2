...
//                            .NOTICE_DTYPE_NATIVE);
    ...
    @RequestMapping(value = "/thirdLogin3", method = RequestMethod.POST)
    public String ThirdLogin2(@ModelAttribute("decodedParams") JSONObject
                                      params, ModelMap model) {
        ResultDict result = ResultDict.SUCCESS;
        String qq = params.getString("qq");
        String wx = params.getString("wx");
        ...
        try {
            ...
            if (StringUtils.isNotBlank(map.get("birthyear").toString())) {
                appUser.getTpadUser().setBirthyear((Integer) map.get
                        ("birthyear"));
            }
...
//            }
            ...
            if (StringUtils.isNotBlank(qq)) {
                if (StringUtils.isBlank(appUser.getIcon()) &&
                        StringUtils.isNotBlank(mapThirdInfo2.get
                                ("qq_image_url").toString())) {

                    appUser.setIcon(mapThirdInfo2.get("qq_image_url")
                            .toString());

                }
                if (StringUtils.isBlank(appUser.getNickname()) &&
                        StringUtils.isNotBlank(mapThirdInfo2.get
                                ("qq_nickname").toString())) {
                    appUser.setNickname(mapThirdInfo2.get("qq_nickname")
                            .toString());
                }
            } else if (StringUtils.isBlank(appUser.getIcon()) && StringUtils
                    .isNotBlank(wx)) {
                if (StringUtils.isNotBlank(mapThirdInfo2.get("wx_image_url")
                        .toString())) {
                    appUser.setIcon(mapThirdInfo2.get("wx_image_url")
                            .toString());
                }
                if (StringUtils.isBlank(appUser.getNickname()) &&
                        StringUtils.isNotBlank(mapThirdInfo2.get
                                ("wx_nickname").toString())) {
                    appUser.setNickname(mapThirdInfo2.get("wx_nickname")
                            .toString());
                }
            }
