...
public class MobileContoller extends BaseDecodedController{
	...
	public String change(@ModelAttribute("decodedParams")JSONObject params, ModelMap model) {
		try {
			...
			accountManager.changeMobile(OfferFactory.generateLoginedMobileValidateOffer(mobile, code, uid, token));
	...
	public String setup(@ModelAttribute("decodedParams")JSONObject params, ModelMap model) {
		try {
			String mobile = params.getString("mobile");
			String code = params.getString("code");
			String uid = params.getString("uid");
			...
			accountManager.setupMobile(OfferFactory.generateLoginedMobileValidateOffer(mobile, code, uid, token), password);
	...
	private void submitRegisterTask(String uid, String token) {
		try {
			userManager.submitTask(OfferFactory.generateUserCommitOffer(Constants.TASK_REGISTER, uid, token, "reg" + uid));
			noticeService.pushMessage(Constants.NOTICE_REGISTER_NAME, Constants.NOTICE_REGISTER_CONTENT, Constants.NOTICE_TYPE_SYSTEM, uid, Constants.NOTICE_DTYPE_NATIVE);
		} catch(TaskRepeatException | CheckNotAllowedException e){
			system.warn("method:submitRegisterTask,uid:" + uid + ",cause:taskRepeatException");
		} catch (Exception e) {
